# Week 7 · CNN from scratch: CIFAR-10
...
Kaggle competition: the link is given at the lab. Download `train.npz` and `test.npz` (about 170 MB) from its *Data* tab into `labs/week_07_cifar10_cnn/data/kaggle/`.
...
Train the best CNN you can from scratch (no pretrained weights), log every run to Weights & Biases (project `zneus-2026`, run names `week07-...`) and submit your predictions for `test.npz` to Kaggle (`submission.csv`, columns `id,label`, 10 000 rows). Target: at least 80 %.
